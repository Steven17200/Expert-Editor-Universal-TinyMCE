// ==UserScript==
// @name         Expert Editor Universal TinyMCE - ULTIMATE V5.5.8
// @namespace    https://github.com/Steven17200
// @version      5.5.8
// @description  FIX ERREUR SYNTAXE + Odysee Autoplay Muted + Restauration Totale des fonctions
// @author       Steven17200
...
(function() {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                try {
                    ...
                    ed.focus();
                    if (fullContent) {
                        let finalHtml = result.replace(/[*_#~]/g, "").replace(/\n\s*\n/g, '\n').split('\n').map(line => {
                ...
                } catch(e) { btn.innerHTML = '❌'; btn.style.color = "#e74c3c"; }
                setTimeout(() => { btn.innerHTML = originalLabel; btn.style.color = (finalLabel === 'US') ? '#ffb300' : (finalLabel === '🧐 Analyse') ? '#3498db' : '#000'; }, 2000);
            }
        });
    }
    ...
    function setupEditor(ed) {
        ...
        toolbar.appendChild(create('btn-ai-analyze', '🧐 Analyse', () => {
            const promptAnalyse = `Tu es rédacteur magazine expert. Identifie l'auteur. 1. POLITESSES : Remercie l'auteur au début : @Auteur. 2. TITRE : Centré anthracite. 3. CITATIONS : Surligne bleu style Humain. 4. MON AVIS : Comic Sans MS.`;
            appelMistral(ed, document.getElementById('btn-ai-analyze'), promptAnalyse, '🧐 Analyse', true);
        }));
        ...
        toolbar.appendChild(create('btn-grid', '📅 Table', () => {
            ...
            if (r && c) {
                let h = '<table style="border-collapse: collapse; width: 100%; border: 1px solid #ccc;">';
        ...
        toolbar.appendChild(create('btn-yt-video', '📺 YouTube', () => {
            ...
            if(id){ ed.focus(); ed.execCommand('mceInsertContent', false, `<div style="display:flex; justify-content:center; margin:10px 0;"><iframe width="560" height="315" src="https://www.youtube.com/embed/${id}" frameborder="0" allowfullscreen style="border-radius:12px; background:#000;"></iframe></div><p></p>`); }
        }));

        toolbar.appendChild(create('btn-odysee', '🚀 Odysee', () => {
            const url = prompt("Lien Odysee (Flux MP4) :");
            if(url && url.includes('.mp4')){
                // Autoplay au survol + Muet (muted) pour garantir le fonctionnement
                ed.focus(); ed.execCommand('mceInsertContent', false, `<div style="display:flex; justify-content:center; margin:10px 0;"><video width="560" height="315" controls muted style="border-radius:12px; background:#000;" onmouseover="this.play()" onmouseout="this.pause(); this.currentTime=0;"><source src="${url}" type="video/mp4">Navigateur non supporté.</video></div><p></p>`);
            }
        }));
        ...
        toolbar.appendChild(create('btn-yt-shorts', '🎬 SHORTS', () => {
            ...
            if(id){ ed.focus(); ed.execCommand('mceInsertContent', false, `<div style="display:flex; justify-content:center; margin:10px 0;"><iframe width="315" height="560" src="https://www.youtube.com/embed/${id}" frameborder="0" allowfullscreen style="border-radius:12px; background:#000;"></iframe></div>`); }
        }));
        ...
        toolbar.appendChild(create('btn-sc', '☁️ SC', () => {
            ...
            if(url){ ed.focus(); ed.execCommand('mceInsertContent', false, `<iframe width="100%" height="166" src="https://w.soundcloud.com/player/?url=${encodeURIComponent(url)}"></iframe>`); }
        }));
